...
OUT="$ROOT_DIR/evidence/USSL_$RUN_ID"
RUN_START_UTC="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"

# Generate self-signed server cert
openssl genrsa -out "$CASE_DIR/wrong_server.key" 4096
openssl req -new -sha256 \
    -key "$CASE_DIR/wrong_server.key" \
    -subj "/C=US/O=Lab Root CA/CN=lab-self-signed-server-leaf" \
    -out "$CASE_DIR/wrong_server.csr"
openssl x509 -req -sha256 \
		-in "$CASE_DIR/wrong_server.csr" \
		-signkey "$CASE_DIR/wrong_server.key" \
		-days 365 \
		-out "$CASE_DIR/wrong_server.crt" \
		-extfile "$CERTS_DIR/server/server.ext" \
		-extensions v3_server
cat "$CASE_DIR/wrong_server.crt" "$CERTS_DIR/intermediate/intermediate.crt" > "$CASE_DIR/wrong_server.fullchain.crt"

# Bring down root nginx
(cd "$ROOT_DIR" && docker compose -f docker-compose.yml stop nginx) >/dev/null 2>&1 || true

# Bring up nginx for this case with self-signed server cert
...
echo "TrustAnchorResolutionFailure/UntrustedSelfSignedLeafCase" > "$OUT/metadata/run_case.txt"

# Client presents certs normally
