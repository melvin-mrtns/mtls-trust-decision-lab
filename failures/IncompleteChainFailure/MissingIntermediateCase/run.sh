...
OUT="$ROOT_DIR/evidence/MI_$RUN_ID"
...
echo "IncompleteChainFailure/MissingIntermediateCase" > "$OUT/metadata/run_case.txt"

# Present client leaf only (omit intermediate)
VERIFIER_CMD=$(cat <<EOF
curl -v \
  --cacert "$CERTS_DIR/ca/root.crt" \
  --cert   "$CERTS_DIR/client/client.crt" \
