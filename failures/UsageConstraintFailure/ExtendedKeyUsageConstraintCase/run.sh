...
OUT="$ROOT_DIR/evidence/EKUC_$RUN_ID"
...
echo "UsageConstraintFailure/ExtendedKeyUsageConstraintCase" > "$OUT/metadata/run_case.txt"

# Generate client cert with wrong EKU role (serverAuth instead of clientAuth)
openssl genrsa -out "$CASE_DIR/wrong_eku_client.key" 2048
openssl req -new -sha256 \
  -key "$CASE_DIR/wrong_eku_client.key" \
  -subj "/C=US/O=Lab Client/CN=lab-client-wrong-eku-constraint" \
  -out "$CASE_DIR/wrong_eku_client.csr"
openssl x509 -req -sha256 \
  -in "$CASE_DIR/wrong_eku_client.csr" \
  -CA "$CERTS_DIR/intermediate/intermediate.crt" \
  -CAkey "$CERTS_DIR/intermediate/intermediate.key" \
  -CAcreateserial \
  -days 120 \
  -out "$CASE_DIR/wrong_eku_client.crt" \
  -extfile "$CASE_DIR/wrong_eku_client.ext"\
  -extensions v3_client
cat "$CASE_DIR/wrong_eku_client.crt" "$CERTS_DIR/intermediate/intermediate.crt" > "$CASE_DIR/wrong_eku_client.fullchain.crt"


# Present the right chain but with extendedKeyUsage=serverAuth client cert
VERIFIER_CMD=$(cat <<EOF
curl -v \
  --cacert "$CERTS_DIR/ca/root.crt" \
  --cert   "$CASE_DIR/wrong_eku_client.fullchain.crt" \
  --key    "$CASE_DIR/wrong_eku_client.key" \
