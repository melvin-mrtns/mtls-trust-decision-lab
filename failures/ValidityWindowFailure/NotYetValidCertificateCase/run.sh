...
OUT="$ROOT_DIR/evidence/NYVC_$RUN_ID"
RUN_START_UTC="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"

# Bring down root nginx
(cd "$ROOT_DIR" && docker compose -f docker-compose.yml stop nginx) >/dev/null 2>&1 || true

# Bring up nginx for this case that has a fake time of 2 days in the past (case-specific compose)
...
echo "ValidityWindowFailure/NotYetValidCertificateCase" > "$OUT/metadata/run_case.txt"

# Create client key + CSR
openssl genrsa -out "$CASE_DIR/client_notyetvalid.key" 2048

openssl req -new -sha256 \
  -key "$CASE_DIR/client_notyetvalid.key" \
  -subj "/C=US/O=Lab Client/CN=lab-client-notyetvalid" \
  -out "$CASE_DIR/client_notyetvalid.csr"

# Create trusted but NOT YET valid client cert
# NotBefore = 2 days in the future, validity length 120 days
# Use faketime to create cert dated 2 days in the future
FAKETIME="+2d" openssl x509 -req -sha256 \
  -in "$CASE_DIR/client_notyetvalid.csr" \
  -CA "$CERTS_DIR/intermediate/intermediate.crt" \
  -CAkey "$CERTS_DIR/intermediate/intermediate.key" \
  -CAcreateserial \
  -out "$CASE_DIR/client_notyetvalid.crt" \
  -extfile "$CERTS_DIR/client/client.ext" \
  -extensions v3_client \
  -days 120

cat "$CASE_DIR/client_notyetvalid.crt" "$CERTS_DIR/intermediate/intermediate.crt" > "$CASE_DIR/client_notyetvalid.fullchain.crt"

# Present not-yet-valid but trusted client cert
VERIFIER_CMD=$(cat <<EOF
curl -v \
  --cacert "$CERTS_DIR/ca/root.crt" \
  --cert   "$CASE_DIR/client_notyetvalid.fullchain.crt" \
  --key    "$CASE_DIR/client_notyetvalid.key" \
