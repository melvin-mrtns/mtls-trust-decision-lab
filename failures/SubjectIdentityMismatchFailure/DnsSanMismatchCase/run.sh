...
OUT="$ROOT_DIR/evidence/DSM_$RUN_ID"
...
echo "SubjectIdentityMismatchFailure/DnsSanMismatchCase" > "$OUT/metadata/run_case.txt"

# Access hostname "wronghost" (mapped to 127.0.0.1 with the resolve flag) which
# should not match the server certificate SAN
VERIFIER_CMD=$(cat <<EOF
curl -v \
  --resolve wronghost:8443:127.0.0.1 \
  --cacert "$CERTS_DIR/ca/root.crt" \
  --cert   "$CERTS_DIR/client/client.fullchain.crt" \
  --key    "$CERTS_DIR/client/client.key" \
  https://wronghost:8443/
EOF
)
